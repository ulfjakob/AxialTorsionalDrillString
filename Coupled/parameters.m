...
p.ka   = 1 *1;                      %[-] Axial damping
p.kt   = 1 *1;                      %[-] Torsional damping
p.Mb = 12e3*0; %[kg] BHA mass
p.Ib = 41;   %[kgm2] BHA moment of inertia  

p.ka_L = 1;     % Reflection coefficient
p.kt_L = 1;     % Reflection coefficient

%% computed quantities
p.c_a = sqrt(p.E/p.rho);      %[m/s] Axial wave velocity 
p.c_t = sqrt(p.G/p.rho);      %[m/s] Torsional wave velocity
...
%% Misc params
p.P   = 1000;      % Number of cells in discretization
...
% BRI
p.omegaMAX = p.omega0*5; % for enforcing CFL condition
dxl = dt*p.omegaMAX;
p.Pl = floor(1/dxl);
% p.Pl = 100;
